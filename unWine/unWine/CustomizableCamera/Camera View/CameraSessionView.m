...
@property (nonatomic) CameraFlash state;
...
- (void)setupCaptureManager:(CameraType)camera {
    ...
    if(self.captureManager && self.captureManager.captureSession && [self.captureManager.captureSession.inputs count] > 0) {
    ...
    if (_captureManager) {
        ...
        if(self.videoEnabled)
            [_captureManager addMovieFileOutput];
...
-(void)composeInterface {
    ...
    if (_captureManager) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(inputManager2:)];
        [tap setNumberOfTapsRequired:1];
        
        UILongPressGestureRecognizer *longTap = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(inputManagerHeld:)];
        ...
        [_cameraShutter addGestureRecognizer:tap];
        [_cameraShutter addGestureRecognizer:longTap];
    ...
    if (_topBarView) {
        ...
        _topBarView.backgroundColor = UNWINE_RED_70;//[UIColor colorWithRed: 0.176 green: 0.478 blue: 0.529 alpha: 0.64];
        [self addSubview:_topBarView];
        
        //Add the flash button
        _state = CameraFlashOff;
...
-(void)inputManager2:(UIGestureRecognizer *)sender {
...
- (void)inputManagerHeld:(UILongPressGestureRecognizer *)recognizer {
    if (_animationInProgress)
        return;
    
    if(!self.videoEnabled) {
        [self inputManager2:recognizer];
        return;
    }
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        [self animateHideNavBar];
        [self animateShutterIncrease:^{
            [self animateRecordingCircle];
            [self performSelector:@selector(stopCapturing) withObject:nil afterDelay:10];
            dispatch_async(dispatch_get_main_queue(), ^{
                [_captureManager captureVideoData];
            });
        }]; //add 10s timer to call stop
    }
...
- (void)stopCapturing {
    [self animateShowNavBar];
    [self animateShutterDecrease];
    self.captureManager.enableTorch = NO;
    [_captureManager stopCaptureVideoData];
}
...
- (void)onTapShutterButton {
    ...
    if(_state == CameraFlashAuto) {
        self.captureManager.enableTorch = YES;
...
- (void)onTapFlashButton {
    if(_state == CameraFlashOff) {
        _state = CameraFlashAuto;
        [_cameraFlash setAlpha:1];
    } else if(_state == CameraFlashAuto) {
        _state = CameraFlashOn;
        self.captureManager.enableTorch = YES;
    } else if(_state == CameraFlashOn) {
        _state = CameraFlashOff;
        self.captureManager.enableTorch = NO;
        [_cameraFlash setAlpha:.3];
    }
...
- (void)onTapDismissButton {
    ...
    if([_delegate respondsToSelector:@selector(willDismissCameraSessionView:)])
        [_delegate willDismissCameraSessionView:YES];
    else
...
- (void)animateHideNavBar {
    [UIView animateWithDuration:.3 animations:^{
        [_topBarView setFrame:CGRectMake(0, -HEIGHT(_topBarView), WIDTH(_topBarView), HEIGHT(_topBarView))];
    }];
}

- (void)animateShowNavBar {
    [UIView animateWithDuration:.1 animations:^{
        [_topBarView setFrame:CGRectMake(0, 0, WIDTH(_topBarView), HEIGHT(_topBarView))];
    }];
}

- (void)animateShutterIncrease:(void(^)(void))completion {
    _animationInProgress = YES; //Disables input manager
    
    [UIView animateWithDuration:.6 animations:^{
        _cameraShutter.transform = CGAffineTransformMakeScale(1.5, 1.5);
    } completion:^(BOOL finished) {
...
- (void)animateRecordingCircle {
    int radius = SEMIWIDTH(_cameraShutter) / 2;
    circle = [CAShapeLayer layer];
    // Make a circular shape
    circle.path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, 2.0*radius, 2.0*radius)
                                             cornerRadius:radius].CGPath;
    // Center the shape in self.view
    circle.position = CGPointMake(CGRectGetMidX(_cameraShutter.frame),
                                  CGRectGetMidY(_cameraShutter.frame));
    
    // Configure the apperence of the circle
    circle.fillColor = [UIColor grayColor].CGColor;
    circle.strokeColor = [UNWINE_RED CGColor];
    circle.lineWidth = 5;
    
    // Add to parent layer
    [_cameraShutter.layer addSublayer:circle];
...
- (void)animateShutterDecrease {
    _animationInProgress = YES; //Disables input manager
    
    if([circle superlayer])
        [circle removeFromSuperlayer];
    
    [_cameraShutter.layer removeAllAnimations];
    [UIView animateWithDuration:.1 animations:^{
        _cameraShutter.transform = CGAffineTransformMakeScale(1.0, 1.0);
    } completion:^(BOOL finished) {
        _animationInProgress = NO; //Enables input manager
    }];
}
...
- (void)cameraSessionManagerDidCaptureMovie {
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(didCaptureMovieWithData:url:thumbnail:)])
            [self.delegate didCaptureMovieWithData:[[self captureManager] movieData] url:[[self captureManager] movieURL] thumbnail:[[self captureManager] movieThumbnail]];
    }
}
...
-(void)cameraSessionManagerFailedToCaptureMovie {
}
